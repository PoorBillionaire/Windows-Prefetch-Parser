...
class Getprefetch(object):
    ...
    def findpf(self):
        try:
            self.prefetch_file_list = self.conn.listPath(
                                                         'C$',
                                                         '/Windows/Prefetch',
                                                         pattern="*.pf"
                                                        )
            ...
            self.conn.close()
            sys.exit("[-] Unable to find Prefetch files at the location specified")

    def download_pf(self):
        if self.prefetch_file_list:
            print "[+] Found Prefetch files"
        else:
            self.conn.close()
            sys.exit("No Prefetch files found on {}".format(host))

        try:
            os.mkdir("PF-{}".format(self.host))
        except Exception, e:
            sys.exit(e)
            self.conn.close()

        try:
            for i in self.prefetch_file_list:
                f = tempfile.NamedTemporaryFile()
                file_attributes, filesize = self.conn.retrieveFile(
                                                                   "C$", 
                                                                   "/Windows/Prefetch/{}".format(i.filename),
                                                                   f
                                                                  )
                f.seek(0)

                with open("PF-{0}/{1}".format(self.host, i.filename), "wb+") as newfile:
                    for i in f.read():
                        newfile.write(i)
            print "[+] Copied Prefetch files succesfully"
            self.conn.close()
